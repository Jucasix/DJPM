...
@Composable
fun MainGameScreen(context: Context, onPlayClick: () -> Unit, onHighScoreClick: () -> Unit) {
    ...
    Canvas(
        ...
        // Desenhar a relva na parte inferior do ecrã
        val grassSrcRect = android.graphics.Rect(0, groundImage.height - 64, 128, groundImage.height) // Seção da relva
        val groundYPosition = size.height - 64.dp.toPx() // Posição do chão na parte inferior

        for (i in 0 until (size.width / 128.dp.toPx()).toInt() + 1) {
            drawIntoCanvas { canvas ->
                canvas.nativeCanvas.drawBitmap(
                    groundImage.asAndroidBitmap(),
                    grassSrcRect,
                    android.graphics.RectF(i * 128.dp.toPx(), groundYPosition, (i + 1) * 128.dp.toPx(), size.height),
                    null
                )
            }
        }
